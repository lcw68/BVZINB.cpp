...
long double l2_A (int x, int a0, int a1, int a2, int k, double adjj = 0) 
{
	return(exp(lgamma(x +a0 -k) + lgamma(k + a1) - lgamma(a0) - lgamma(x-k+1) - lgamma(a1) - lgamma(k+1) - adjj));
}
long double l3_A (int y, int a0, int a1, int a2, int m, double adjj = 0) 
{
...
long double *dBvZINB4_Expt(int x, int y, int a0, int a1, int a2, int b1, int b2, double p1, double p2, double p3, double p4) 
{
	...
	// expectation components
	 ...
	 double E_E1 = sum_AC * exp(adj_sum) * l1_B;
	  ...
	  E_E4 = E_E4/su;
      //E_E{2} <- E.E/sum(E.E)
      long double v_E = y==0?0:y + (a0 + a2) * b2 *(E_E2 + E_E3 +E_E4);
